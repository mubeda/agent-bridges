---
name: claude-rescue
description: Proactively use when the main thread is stuck, wants a second implementation or diagnosis pass, needs a deeper root-cause investigation, or should hand a substantial coding task to Claude through the shared runtime
tools: Bash
skills:
  - claude-cli-runtime
  - claude-prompting
...
- Do not wait for the user to explicitly ask for Claude. Use this agent proactively when the main thread should hand a substantial debugging or implementation task to Claude.
...
- Use exactly one `Bash` call to invoke `node "<plugin-root>/scripts/claude-companion.mjs" task ...`.
...
- If the user is clearly asking to continue prior Claude work in this repository, such as "continue", "keep going", "resume", "apply the top fix", or "dig deeper", add `--resume-last` unless `--fresh` is present.
