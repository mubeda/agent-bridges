...
description: Run a coding task with a strict manager/implementer split - Claude makes every repository change while you plan, coordinate parallel chunks, code-review every diff, and commit. Use ONLY when the user explicitly invokes the claude-delegate skill or explicitly asks to delegate implementation to Claude ("delegate this to claude", "have claude implement this", "claude implements, you review"). Never auto-trigger for ordinary coding requests that don't name Claude delegation.
user-invocable: true
---

# Delegate to Claude

You are the **manager and code reviewer**. Claude is the **implementer**. This
...
Every Claude run goes through the companion's `task`, `status`, and `result`
subcommands; never invoke the backend CLI directly.

## Hard rules

1. **Every repository file change goes through Claude** — code, tests, docs,
   fixtures, configs, reports, all of it. No exception for "trivial" fixes:
   a one-line lint fix found in review goes back to Claude like anything
   ...
   writing, hand Claude your evidence notes and let it write the file.
2. **Claude never commits.** An uncommitted worktree is your review
   boundary: `git status` must show exactly the unreviewed work. Tell
   Claude this in every prompt.
3. **Claude never touches protected paths.** Three layers, all restated
...
5. **Claude reports are claims, not evidence.** Verify every claim yourself
   before acting on it.
...
```markdown
# claude-delegate docket — <task> — <date>
...
```bash
cd <repo> && node "<plugin-root>/scripts/claude-companion.mjs" task --wait --fresh --write "$(cat <<'CHUNK_PROMPT'
<chunk prompt>
CHUNK_PROMPT
)"
```

- **Always `--fresh --write` for implementation.** `--write` runs Claude
  with permission checks bypassed (`--dangerously-skip-permissions`);
  without it the run stays in plan mode and produces no edits. The user
  must have run `claude --dangerously-skip-permissions` once interactively
  to acknowledge the bypass disclaimer — if a launch fails on that
  disclaimer, stop and tell the user. `--fresh` keeps every chunk on its own thread: the companion can
  only resume the latest thread in a workspace, so with parallel chunks in
  flight a resume may target the wrong one.
- **Always `--wait`, never `--background`, for implementation.** `claude
  --bg` inside a git repo may isolate the job in `.claude/worktrees/`, so
  its edits would never reach the worktree you review. `--wait` runs in
  place; for concurrency, start each blocking launch in a background host
  shell instead.
- `cd <repo>` matters: the companion keys its job state to the working
  directory.
- If the companion or the `claude` CLI is missing or unauthenticated, stop
  and tell the user to run setup (the `claude-setup`
  skill).
...
locations, prior decisions — everything Claude would otherwise re-derive>
...
2. **Boundaries**: HEAD unchanged (no commits by Claude); no protected
   path touched.
3. **Read every diff hunk.** Claude's report describes intent; only the
...
5. **Re-run every suite Claude claims green, yourself.** Also run the
...
(e.g. no loopback binds, so socket tests fail there) — a failure Claude
reports as environmental must be re-run locally before you believe either
way.

### 5. Findings: re-feed, at most twice

Every finding — however small — goes back to Claude with precise
...
findings, and what Claude did instead. Update the docket's re-feed count
each time.
...
Implemented by Claude (claude-delegate) under supervision; reviewed,
...
findings: diagnose, then re-feed the responsible chunk's fix to Claude
...
If the companion is missing, the `claude` CLI is unauthenticated, or jobs
