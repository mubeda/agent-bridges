...
function formatOpencodeResumeCommand(job) {
  ...
  return `opencode run -s ${job.threadId}`;
}
...
function formatSetupModelLine(model) {
  ...
  if (model.source === "error") {
    return `unavailable (${model.detail || "could not read opencode config"})`;
  }
  return model.detail;
}
...
export function renderStoredJobResult(job, storedJob) {
  ...
  const resumeCommand = threadId ? `opencode run -s ${threadId}` : null;
