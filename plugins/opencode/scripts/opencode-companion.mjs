...
import { importToOpencode } from "./lib/transfer-dest.mjs";

const ROOT_DIR = path.resolve(fileURLToPath(new URL("..", import.meta.url)));
const REVIEW_SCHEMA = path.join(ROOT_DIR, "schemas", "review-output.schema.json");
const DEFAULT_STATUS_WAIT_TIMEOUT_MS = 240000;
const DEFAULT_STATUS_POLL_INTERVAL_MS = 2000;
const VALID_REASONING_EFFORTS = new Set(["minimal", "low", "medium", "high", "max"]);
...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/opencode-companion.mjs task [--background] [--resume-last|--resume|--fresh] [--model <provider/model>] [--effort <minimal|low|medium|high|max>] [prompt]",
      "  node scripts/opencode-companion.mjs review-worker --cwd <path> --job-id <id>",
      ...
      "  node scripts/opencode-companion.mjs transfer [--source <path>] [--json]"
    ].join("\n")
  );
}
...
function normalizeReasoningEffort(effort) {
  ...
  if (!VALID_REASONING_EFFORTS.has(normalized)) {
    throw new Error(
      `Unsupported reasoning effort "${effort}". Use one of: minimal, low, medium, high, max.`
    );
  }
  return normalized;
}
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  const authStatus = await getOpencodeAuthStatus(cwd);
  const modelStatus = opencodeStatus.available
    ? getOpencodeDefaultModel(cwd)
    : {
        configured: false,
        model: null,
        source: "unavailable",
  ...
  if (!opencodeStatus.available) {
    nextSteps.push("Install OpenCode with `npm install -g opencode-ai`.");
  }
  if (opencodeStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `!opencode auth login` to add a provider.");
  }
  if (!config.stopReviewGate) {
    nextSteps.push(
      "Optional: run `/opencode:setup --enable-review-gate` to require a fresh review before stop."
    );
  }
...
function ensureOpencodeAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("OpenCode CLI is not installed or is missing required runtime support. Install it with `npm install -g opencode-ai`, then rerun `/opencode:setup`.");
  }
}
...
function validateReviewRequest(_target, focusText, { reviewName }) {
  if (reviewName === "Review" && focusText.trim()) {
    throw new Error(
      `\`/opencode:review\` does not accept custom focus text. Retry with \`/opencode:adversarial-review ${focusText.trim()}\` for focused review instructions.`
    );
  }
}
...
function findLatestResumableTaskJob(jobs) {
  // Only treat finished-and-still-extant sessions as resumable. Cancelled
  // jobs deleted their OpenCode session via `opencode session delete`, so
  // resuming their threadId would always fail immediately.
  return (
    jobs.find(
      (job) =>
        job.jobClass === "task" &&
...
async function executeTaskRun(request) {
  ...
  if (request.resumeLast) {
    ...
    if (!latestThread) {
      throw new Error("No previous OpenCode task thread was found for this repository.");
    }
...
function buildReviewJobMetadata(reviewName, target) {
  return {
    ...
    title: reviewName === "Review" ? "OpenCode Review" : `OpenCode ${reviewName}`,
  ...
  if (interrupt.attempted) {
    appendLogLine(
      ...
      interrupt.interrupted
        ? `Requested OpenCode session interrupt for ${threadId}.`
        : `OpenCode session interrupt failed${interrupt.detail ? `: ${interrupt.detail}` : "."}`
    );
  }
...
function renderTransferResult(payload) {
  const lines = [
    "Transferred the host transcript into an OpenCode session.",
    `Source (${payload.host}): ${payload.sourcePath}`,
    `OpenCode session ID: ${payload.sessionId}`,
    `Resume in OpenCode: ${payload.resumeCommand}`
  ];
  return `${lines.join("\n")}\n`;
}
...
async function handleTransfer(argv) {
  ...
  // packed enforces the 100_000-char transcript budget; import the capped messages.
  if (packed.length === 0 && capped.length > 0) {
    throw new Error("Transfer packing produced an empty transcript.");
  }

  const imported = await importToOpencode(cwd, capped, {
    title: `Transfer from ${resolved.host}`
  });
  ...
  const payload = {
    ...
    resumeCommand: imported.resumeCommand
  };
