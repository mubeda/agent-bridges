...
```bash
timeout 600 bash -c 'until node "<plugin-root>/scripts/opencode-companion.mjs" status <job-id> --json | grep -qE "\"status\": *\"(completed|failed|cancelled)\""; do sleep 30; done'
```
...
1. For every in-flight chunk, run
   `node "<plugin-root>/scripts/opencode-companion.mjs" status <job-id> --json`
...
`node "<plugin-root>/scripts/opencode-companion.mjs" result <job-id>` and move
it to review.
