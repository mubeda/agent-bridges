...
export function resolveTransferSource(cwd, options = {}) {
  ...
  const preferClaude = Boolean(env.CLAUDE_PROJECT_DIR) || claudeFiles.length > 0;
  const preferCodex = Boolean(env.CODEX_HOME) || codexFiles.length > 0;

  let selected = null;
  let host = null;

  if (preferClaude && claudeFiles.length > 0) {
    selected = newestFile(claudeFiles, options);
    host = "claude";
  } else if (preferCodex && codexFiles.length > 0) {
    ...
    const all = [
      ...claudeFiles.map((filePath) => ({ filePath, host: "claude" })),
      ...codexFiles.map((filePath) => ({ filePath, host: "codex" })),
      ...cursorFiles.map((filePath) => ({ filePath, host: "cursor" }))
    ];
    if (all.length > 0) {
      let best = all[0];
      let bestMtime = getMtimeMs(best.filePath, options);
      for (let i = 1; i < all.length; i += 1) {
        const mtime = getMtimeMs(all[i].filePath, options);
        if (mtime >= bestMtime) {
          best = all[i];
          bestMtime = mtime;
        }
      }
      selected = best.filePath;
      host = best.host;
    }
  }
