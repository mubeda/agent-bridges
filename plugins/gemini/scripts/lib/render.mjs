...
export function formatResumeCommand(backend, threadId) {
  if (backend === "agy") {
    return `agy --conversation ${threadId}`;
  }
  return `gemini -r ${threadId}`;
}
...
function formatGeminiResumeCommand(job) {
  ...
  return formatResumeCommand(job.backend ?? "gemini", job.threadId);
}
...
function formatSetupModelLine(model) {
  ...
  if (model.source === "error") {
    return `unavailable (${model.detail || "could not read gemini config"})`;
  }
  return model.detail;
}
...
export function renderSetupReport(report) {
  ...
  const modelLine = formatSetupModelLine(report.model);
  if (modelLine) {
  ...
  lines.push(
    ...
    `- review gate: ${report.reviewGateEnabled ? "enabled (Claude Stop hook only)" : "disabled"}`,
    ""
  );
...
export function renderStoredJobResult(job, storedJob) {
  ...
  const backend = job.backend ?? storedJob?.backend ?? "gemini";
  const resumeCommand = threadId ? formatResumeCommand(backend, threadId) : null;
