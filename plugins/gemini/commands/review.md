---
name: review
description: Run an Gemini code review against local git state
argument-hint: '[--wait|--background] [--base <ref>] [--scope auto|working-tree|branch]'
disable-model-invocation: true
allowed-tools: Read, Glob, Grep, Bash(node:*), Bash(git:*), AskUserQuestion
---

Run an Gemini review through the shared plugin runtime.
...
- If the raw arguments include `--background`, do not ask. Run the review through the companion's background mode.
...
- `/gemini:review` does not support staged-only review, unstaged-only review, or extra focus text.
