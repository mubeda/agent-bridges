...
```bash
until node "<plugin-root>/scripts/gemini-companion.mjs" status <job-id> --json | grep -qE '"status": *"(completed|failed|cancelled)"'; do sleep 30; done
```

When a chunk finishes, pull the report with
`node "<plugin-root>/scripts/gemini-companion.mjs" result <job-id>`.
