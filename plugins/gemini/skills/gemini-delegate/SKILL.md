...
description: Run a coding task with a strict manager/implementer split - Gemini makes every repository change while you plan, coordinate parallel chunks, code-review every diff, and commit. Use ONLY when the user explicitly invokes /gemini:delegate or explicitly asks to delegate implementation to Gemini ("delegate this to gemini", "have gemini implement this", "gemini implements, you review"). Never auto-trigger for ordinary coding requests that don't name Gemini delegation.
user-invocable: true
---

# Delegate to Gemini

You are the **manager and code reviewer**. Gemini is the **implementer**. This
...
Every Gemini run goes through the companion's `task`, `status`, and `result`
subcommands; never invoke the backend CLI directly.

## Hard rules

1. **Every repository file change goes through Gemini** — code, tests, docs,
   fixtures, configs, reports, all of it. No exception for "trivial" fixes:
   a one-line lint fix found in review goes back to Gemini like anything
   ...
   writing, hand Gemini your evidence notes and let it write the file.
2. **Gemini never commits.** An uncommitted worktree is your review
   boundary: `git status` must show exactly the unreviewed work. Tell
   Gemini this in every prompt.
3. **Gemini never touches protected paths.** Three layers, all restated
...
5. **Gemini reports are claims, not evidence.** Verify every claim yourself
   before acting on it.
...
```markdown
# gemini-delegate docket — <task> — <date>
...
```bash
cd <repo> && node "<plugin-root>/scripts/gemini-companion.mjs" task --background --fresh --write "$(cat <<'CHUNK_PROMPT'
<chunk prompt>
CHUNK_PROMPT
)"
```
...
- **Always `--fresh --write` for implementation.** `--write` runs the
  backend with write approvals enabled; without it the run stays in plan
  mode and produces no edits. `--fresh` keeps every chunk on its own thread: the companion can
...
- If the companion or the Gemini backend (`agy` or `gemini`) is missing or unauthenticated, stop
  and tell the user to run setup (`/gemini:setup` or the `gemini-setup`
  skill).
...
locations, prior decisions — everything Gemini would otherwise re-derive>
...
```bash
timeout 600 bash -c 'until node "<plugin-root>/scripts/gemini-companion.mjs" status <job-id> --json | grep -qE "\"status\": *\"(completed|failed|cancelled)\""; do sleep 30; done'
```
...
1. For every in-flight chunk, run
   `node "<plugin-root>/scripts/gemini-companion.mjs" status <job-id> --json`
...
`node "<plugin-root>/scripts/gemini-companion.mjs" result <job-id>` and move
it to review.
...
2. **Boundaries**: HEAD unchanged (no commits by Gemini); no protected
   path touched.
3. **Read every diff hunk.** Gemini's report describes intent; only the
...
5. **Re-run every suite Gemini claims green, yourself.** Also run the
...
(e.g. no loopback binds, so socket tests fail there) — a failure Gemini
reports as environmental must be re-run locally before you believe either
way.

### 5. Findings: re-feed, at most twice

Every finding — however small — goes back to Gemini with precise
...
findings, and what Gemini did instead. Update the docket's re-feed count
each time.
...
Implemented by Gemini (gemini-delegate) under supervision; reviewed,
...
findings: diagnose, then re-feed the responsible chunk's fix to Gemini
...
If the companion is missing, the Gemini backend (`agy` or `gemini`) is unauthenticated, or jobs
